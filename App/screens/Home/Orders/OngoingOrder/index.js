...
import {cancelOrder, commenceOrder, completeOrder} from 'services/orders/actions';
...
export default connect(mapStateToProps, {
  ...
  const _cancelOrder = async () => {
    ...
    try {
      await api({
        url: `/${props.type.toLowerCase()}/cancel/${props.id}`,
        method: 'PUT',
      });
      props.cancelOrder();
      Snackbar.show({
  ...
  return (
    <View style={BaseStyles.background}>
      <DashNav
        showBackButton={['COMPLETE', 'CANCELLED'].includes(props.stage)}
        navigation={props.navigation}
        title={`${
          props.stage === 'COMPLETE'
            ? 'Completed'
            : props.stage === 'CANCELLED'
            ? 'Cancelled'
            : 'Ongoing'
        } Order`}
